...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		{
			Game.Notify(loc("Ignore This Message."),loc("This message confirms the sync with firebase and is used for devtesting.")+'<div class="line"></div><a style="float:right;" onclick="Game.prefs.showBackupWarning=0;==CLOSETHIS()==">'+loc("Don't show this again")+'</a>',[25,7]);
		}
		...
		BAKERY NAME
		=======================================================================================*/
		const storedUsername = localStorage.getItem("eg-username");
		...
		Game.bakeryNameRefresh=function()
		{
			...
			if (!App && name.indexOf('80615',name.length-('80615').length)>0 && !Game.sesame) Game.OpenSesame();
				...
				str+=
					'<div class="block" style="padding:0px;margin:8px 4px;">'+
						...
						'<div class="title">'+loc("General")+
							...
							'<div class="listing"><a class="option smallFancyButton" '+Game.clickStr+'="Game.ExportSave();PlaySound(\'snd/tick.mp3\');">'+loc("Export save")+'</a><label>'+loc("You can use this to backup your save or to transfer it to another computer (shortcut for import: ctrl+O)")+'</label></div>'+
							(!App?('<div class="listing"><a class="option smallFancyButton" '+Game.clickStr+'="Game.FileSave();PlaySound(\'snd/tick.mp3\');">'+loc("Save to file")+'</a><a class="option smallFancyButton" style="position:relative;"><input id="FileLoadInput" type="file" style="cursor:pointer;opacity:0;position:absolute;left:0px;top:0px;width:100%;height:100%;" onchange="Game.FileLoad(event);" '+Game.clickStr+'="PlaySound(\'snd/tick.mp3\');"/>'+loc("Dev Tools")+'</a><label>'+loc("Use this to keep backups on your computer")+'</label></div>'):'')+
